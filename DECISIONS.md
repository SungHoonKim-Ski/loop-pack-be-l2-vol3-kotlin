...
### 보완: Tomcat max-threads 축소 (200 → 50)
- Virtual Threads 활성화 시 Tomcat의 Platform Thread가 carrier thread로만 동작하며, 요청 처리는 Virtual Thread가 담당한다.
- Platform Thread 200개는 Virtual Thread 환경에서 과도하다. carrier thread는 CPU 코어 수 × 2 수준이면 충분하며, 불필요한 Platform Thread는 메모리와 컨텍스트 스위칭 비용만 증가시킨다.
- `server.tomcat.threads.max=50`으로 축소하여 carrier thread 수를 적정화한다.
...
### 보완: 재고 동시성 제어 (비관적 락, Phase 1 포함)
- `ProductService.deductStock()`에서 `SELECT FOR UPDATE`(비관적 락)를 적용하여 동시 주문 시 재고 정합성을 보장한다.
- Phase 1(기능 구현)에 포함한다. 재고 차감은 정합성이 핵심이므로, 기능 완성 후 별도 해결이 아닌 기능 구현 시점에 동시성 제어를 함께 적용한다.
- 비관적 락을 선택한 이유: 재고 차감은 충돌 빈도가 높은(인기 상품) write 연산이며, 낙관적 락(version)의 재시도 비용보다 비관적 락의 대기 비용이 예측 가능하다.
...
### 트레이드오프
- 최대 5분의 수치 지연 → 수용 가능 (좋아요 수의 실시간 정확성은 비크리티컬)
- 배치 실패 시 수치 정체 → 배치 모니터링으로 대응
- 배치 주기 5분 근거: 좋아요 수는 UX에 영향이 적고, 1분 미만은 배치 오버헤드 대비 이점이 적으며, 10분 이상은 사용자 체감 지연이 커진다.
